...
If selectvalue <> "" and instr(selectvalue,":")>0 then
	...
	FOR k = 1 to UBound(selectvalue_value)
		...
		strSQL2 = "SELECT name,gunphone,cellphone,class,armyno,sosok_id,sosok_name FROM TB_SMSADDR where idx = "& selectvalue_value(k) &""
		...
		if Not(rss.eof or rss.bof) Then
			...
			Do While Not RSs.EOF
				...
				if rs2.eof then
					...
					strSQL = "INSERT INTO TB_SMSADDR ( group_idx, name,cellphone,gunphone,class,armyno,sosok_id,sosok_name,incode,indate,mocode,modate)" &_
						" values ("&ngroupidx&",'"& sname	& "', " &_
								"'" & cellphone		& "','" & gunphone		& "', '" & sclass		& "', '" & armyno		& "', '" & sosok_id		& "', '" & sosok_name		& "', '" & INCODE		& "',getdate(), '" & INCODE		& "',getdate())"
...
%>	
		...
		<!--
			...
			self.location.href = 'conferencegroup.asp';
		//-->
		</SCRIPT>
