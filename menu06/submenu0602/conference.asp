...
<%
	...
	if QueryYN = "Y" then
		...
		SQL = SQL & "	WHERE	groupgb = '3' and a.group_idx = g.idx"
		if whereCD1 <> "" then
...
<%
	...
	%>
					<tr onmouseover="this.style.background='#FFFCE7'" bgcolor="#FFFFFF" height="30">
						...
						<td align="center"  width="100"><%=sname%></td>
						<td align="center"  width="70"><%=gunphone%></td>
						<td align="center"  width="100"><%=FormatHPNo(cellphone)%></td>
...
%>
				<tr bgcolor="#FFFFFF" height="30">
					<td align="right" colspan=11 ><img src="/Images/Btn/BtnPlus.gif" style="cursor:hand;" align="absmiddle" title="선택대상 다자간통화목록으로 추가" onclick="fn_rightmove();"></td>
				</tr>
<% end if%>
        	</table>     
		...
		<td width="450" height="750">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
			    <tr height="30">
					<td align="left" bgcolor="#FFFFFF" class="TDCont" colspan="8">&nbsp;<img src="/Images/dot_01.gif" >&nbsp;<b><font color="#ff00ff"></font> 다자간통화대상</b></td>
				</tr>
        	</table>
        	<table width="100%" border="0" cellspacing="1" cellpadding="1" bgcolor="#CCCCCC">
...
<%
			...
			SQL = SQL & "	where	addr_idx = a.idx and datagb = '1' order by c.idx"
				...
				<tr bgcolor="#FFFFFF" height="30">
					<td align="left" colspan=7>
						 ...
						 <tr><td bgcolor="#FFFFFF" align='left'>
								<img src="/Images/Btn/BtnMinus.gif" style="cursor:hand;" align="absmiddle" title="선택대상 다자간통화목록에서 제외" onclick="fn_leftmove();"></td><td align="right" ><img src="/Images/Btn/BtnCallConference1.GIF" style="cursor:hand;" align="absmiddle" onclick="fn_start();"></td>
						</tr></table>
					</td>
				</tr>
<% end if %>
...
<script>
	...
	function fn_rightmove() {
		...
		for(i=0;i<document.all.chk.length;i++) {
				if ( document.all.chk[i].checked )
				{
					...
					//DBFrame.location= "/menu06/submenu0602/conference_move.asp?FRM=right&idx="+document.all.chk[i].value;
				}
		}
		if ( document.all.chk[0] == null && document.all.chk.checked  )
		{
			...
			//DBFrame.location= "/menu06/submenu0602/conference_move.asp?FRM=right&idx="+document.all.chk.value;
		}
		if ( j > 0 )
		{
			DBFrame.location= "/menu06/submenu0602/conference_move.asp?FRM=right&idx="+codestring;
	...
	function fn_leftmove() {
		...
		for(i=0;i<document.all.chk1.length;i++) {
				if ( document.all.chk1[i].checked )
				{
					j = j + 1;
					if ( j == 1 )
						codestring = "1,"+document.all.chk1[i].value;
					else
						codestring = codestring+ ","+document.all.chk1[i].value;
					//DBFrame.location= "/menu06/submenu0602/conference_move.asp?FRM=left&idx="+document.all.chk1[i].value;
				}
		}
		...
		{
			j = j + 1;
			codestring = "1,"+document.all.chk1.value;
			//DBFrame.location= "/menu06/submenu0602/conference_move.asp?FRM=left&idx="+document.all.chk1.value;
		}
		if ( j > 0 )
		{
			document.inUpFrm.QueryYN.value = "Y";
			DBFrame.location= "/menu06/submenu0602/conference_move.asp?FRM=left&idx="+codestring;
			document.inUpFrm.submit();
		}
	...
	function fn_start() {
				...
				conference = window.open("monitor.asp","conference","toolbar=no,status=yes,location=no,width=800,height=600,top=0,left=0,scrollbars=yes,resizable=yes");
				conference.focus();
	}
