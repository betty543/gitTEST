...
<table border="0" cellspacing="0" width="100%">
	<tr height="25"><td class="FBlkB" align="center">3차분류</td></tr>
	<tr>
			...
			%>
				<tr id="cTR<%=i%>" onclick="ClickBG('<%=i%>','<%=trowcount%>');" bgcolor="ffffff">
					...
					<td nowrap width="35" align="center">
						<img src="/Images/Btn/BtnIconModify.gif" title="수정" style="cursor:hand;" align="absmiddle" onClick=" parent.TempleteADD('ProType_Detail.asp?Seq=<%=Seq%>&db_flag=UP&class_gb=C&Aclass=<%=sAclass%>&Bclass=<%=sBclass%>&Cclass=<%=sCclass%>','ON');">
						<img src="/Images/Btn/BtnIconDel.gif" title="삭제" style="cursor:hand;" align="absmiddle" onClick="javascrip:if(confirm('해당 데이타를 삭제 하시겠습니까?')) {location.href='ProType_InsUpDel.asp?Seq=<%=Seq%>&db_flag=DEL&class_gb=C&Aclass=<%=sAclass%>&Bclass=<%=sBclass%>&Cclass=<%=sCclass%>'}">
					</td>
				</tr>
	...
	<% if sBclass <> "" then %>
	<tr><td height="25" align="center"><input type="button" name="BtnPrint" value="3차 추가" style="width:100%; height:100%;" class="Btn4" onClick="parent.TempleteADD('ProType_Detail.asp?db_flag=INS&class_gb=C&Aclass=<%=sAclass%>&Bclass=<%=sBclass%>','ON')"></td></tr>
	<% end if %>
</table>
</div>
