...
<table border="0" cellspacing="0" width="100%">
			...
			%>
				<tr id="cTR<%=i%>" onclick="ClickBG('<%=i%>','<%=trowcount%>');" bgcolor="ffffff">
					...
					<td nowrap width="35" align="center">
						<center>
						<img src="/Images/Btn/BtnIconModify.gif" title="수정" style="cursor:hand;" align="absmiddle" onClick="goPFrame('ProType_5.asp','Pro5fr'); parent.TempleteADD('ProType_Detail.asp?Seq=<%=Seq%>&db_flag=UP&class_gb=D&Aclass=<%=sAclass%>&Bclass=<%=sBclass%>&Cclass=<%=sCclass%>&Dclass=<%=sDclass%>','ON');">
	...
	<% if sCclass <> "" then %>
	<tr><td height="25" align="center"><input type="button" name="BtnPrint" value="4차 추가" style="width:100%; height:100%;" class="Btn4" onClick="goPFrame('ProType_5.asp','Pro5fr'); parent.TempleteADD('ProType_Detail.asp?db_flag=INS&class_gb=D&Aclass=<%=sAclass%>&Bclass=<%=sBclass%>&Cclass=<%=sCclass%>','ON')"></td></tr>
	<% end if %>
</table>
</div>
