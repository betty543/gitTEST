    ...
    <td bgcolor="FFFFFF" align="left" height="29" valign="center">
		...
		<%if level = "A" or  level = "B" then 
				...
				do until RsCode.eof
					...
					SelectedValue = ""
					if j = 1 then
						sReplyHtml = "<input type='RADIO' value='" & RsCode("CODE") & "' name='SOSOKGB3' class='none' " & SelectedValue & "  onClick=""fn_putetc3('"&RsCode("CODE")&"');"">" & RsCode("CODENAME")	
					else
						sReplyHtml = sReplyHtml & "&nbsp;<input type='RADIO' value='" & RsCode("CODE") & "' name='SOSOKGB3' class='none' " & SelectedValue & "  onClick=""fn_putetc3('"&RsCode("CODE")&"');"">" & RsCode("CODENAME")	
					end if
					RsCode.movenext
				loop
				RsCode.close
				...
				sReplyHtml = "계급(대)를 선택하시면 중분류가 표시됩니다."
				response.write sReplyHtml
