...
'response.Write "INS"
	...
	SQL = "select MAX(JUBSEQ) from TB_CRIMECALLHISTORY where LEFT(JUBSEQ,6) = CONVERT(CHAR(6),GETDATE(),112)"
	Rs.open SQL,db
	IF ISNULL(Rs(0)) THEN
	...
	IF REFERJUBSEQ <> "" THEN
		'갯수 파악하기
		SQL = "SELECT COUNT(0) FROM TB_CRIMECALLHISTORY WHERE REFERJUBSEQ ='" & REFERJUBSEQ &"'"
	...
	db.begintrans	
		...
		SQL = " INSERT INTO TB_CRIMECALLHISTORY (JUBSEQ"
		SQL = SQL & "	,	JUBDATE"
		SQL = SQL & "	,	JUBTIME"
		SQL = SQL & "	,	IOFLAG"
		SQL = SQL & "	,	EMERYN"
		...
		if flag = "Y" then
			...
			pageUrl = "/menu03/submenu0321/lifecallhistory.asp"
			Call MsgGoUrl( "정상적으로 등록되었습니다.",pageUrl)
		else
			db.RollBackTrans
			LogWrite "ERROR_SQL="&SQL, "lifecallhistory_InsUpDel.asp", "/menu03/submenu0321/"
...
case "UP"
	...
	flag = "N"			'UPDATE 성공여부
		...
		SQL = " UPDATE TB_CRIMECALLHISTORY SET "
...
'response.write SQL
	...
	if flag = "Y" then
		...
		where1 = where1 & "&whereCD6_B=" & whereCD6_B& "&whereCD6_C=" & whereCD6_C& "&whereCD13_A=" & whereCD13_A& "&whereCD13_B=" & whereCD13_B & "&whereGB=" & whereGB
		where2 = "curPage=" & curPage & "&" & where1

		pageUrl = "/menu03/submenu0321/lifecallhistory.asp?"&where2
...
case "DEL"
	...
	SQL = "DELETE FROM TB_CRIMECALLHISTORY WHERE JUBSEQ = '" & TRIM(JUBSEQ) & "'"
