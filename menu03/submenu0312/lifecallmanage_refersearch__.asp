...
<table border="0" width="940" cellpadding="0" cellspacing="3" bgcolor="#EFECE5" align="center">
	...
	<form method="post" name="inUpFrm" style="margin:0" action="/menu03/submenu0312/lifecallmanage_refersearch.asp">
	<tr bgcolor="#FFFFFF">
		<td>
...
<table border="0" width="940" cellspacing="1" cellpadding="0" width="100%" bgcolor="#CCCCCC" align="center">
	<tr height="25" bgcolor="#EEF6FF" align="center">
		...
		<td rowspan='2' class="TDCont" align='center'><b>상담일시</b></td>
		<td class="TDCont" width="80" align='center'><b>상담유형</b></td>
		<td class="TDCont" width="80" align='center'><b>상담분야</b></td>
		...
		<td rowspan='2' class="TDCont" width="50" align='center'><b>&nbsp;</b></td>
	</tr>
	<tr height="25" bgcolor="#EEF6FF" align="center">
...
<%
	...
	if QueryYN = "Y" then
		...
		sql = " select count(*) + 1 from tb_lifecallhistory where CUSTNAME = '" & db_CUSTNAME & "'"
		set Rs1 = db.execute(sql)
		
		db_TotalREFCNT = Rs1(0)

		if len(db_QUESTION)> 36 then
			db_QUESTION_con = CutString(db_QUESTION, 36, "...")
		else
			db_QUESTION_con = ""
		end if

		if len(db_REPLY)> 36 then
			db_REPLY_con = CutString(db_REPLY, 36, "...")
		else
			db_REPLY_con = ""
		end if
...
%>
		...
		<tr bgcolor="#FFFFFF">
			...
			<td align="center" rowspan='2'><% if db_EMERYN="Y" then%><font color="#0000ff">긴급</font><%end if%></td>
			<td align="center" rowspan='2'><a href="javascript:fn_detail('<%=db_QUESTION%>','<%=db_REPLY%>')"><%=db_JUBTIME1%></a></td>
		...
		<tr bgcolor="#FFFFFF">
			<td align="left" colspan='3' title="<%=db_QUESTION%>">&nbsp;<%=db_QUESTION_Con%></td>
			<td align="left" colspan='4' title="<%=db_REPLY%>">&nbsp;<%=db_REPLY_Con%></td>
		</tr>
<%
			startRow = startRow - 1
			RS.MOVENEXT
