...
db_QUESTION = replace(db_QUESTION,"'","`")
db_QUESTION = replace(db_QUESTION,chr(13)&chr(10),"\n")

db_REPLY = replace(db_REPLY,"'","`")
db_REPLY = replace(db_REPLY,chr(13)&chr(10),"\n")
%>
<script type="text/javascript">
	...
	arrR[<%=i%>] = '<%=db_REPLY%>';
		...
		<tr bgcolor="#FFFFFF">
			...
			<td align="center" rowspan='2'><a href="javascript:fn_detail(<%=i%>)"><%=db_JUBTIME1%></a></td>
		...
		<tr bgcolor="#FFFFFF">
			<td align="left" colspan='3' title="<%=i%>">&nbsp;<%=db_QUESTION_Con%></td>
			<td align="left" colspan='4' title="<%=i%>">&nbsp;<%=db_REPLY_Con%></td>
...
<script>
	...
	function fn_detail(n) {
		//document.all.QUESTION.value = arg1;
		//document.all.REPLY.value = arg2;
		document.all.QUESTION.value = arrQ[n];
		document.all.REPLY.value = arrR[n];
	}
	function fn_Search() {
