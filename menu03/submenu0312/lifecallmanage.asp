...
'response.write RecFileName
		sGotoURL = "/menu03/submenu0302/lifecallmanage.asp?InType=RECORD&RecDate="&RecDate&"&LINEKIND="&LINEKIND&"&telNo="&sCID&"&CallTIME="&CallTIME&"&FileName="&RecFileName
	...
	elseif InType = "CALL" then	'인입콜임.
		...
		sGotoURL = "/menu03/submenu0302/lifecallmanage.asp?InType=CALL&LINEKIND="&LINEKIND&"&telNo="&sCID
...
whereCD6_C = Trim(request("whereCD6_C")) '계급구분
		...
		where1 = where1 & "&whereCD5_A=" & whereCD5_A& "&whereCD5_B=" & whereCD5_B& "&whereCD5_C=" & whereCD5_C& "&whereCD5_D=" & whereCD5_D& "&whereCD5_E=" & whereCD5_E
	...
	if JUBSEQ = "" then
		...
		if rs.eof = false then
			...
			if RECORDFILE = "" and CALLID <> "" then
				...
				RecFileName = RECORDFILE
			end if


			OriginalCustnm = db_GetCustNameA(REFERJUBSEQ)
...
<table border="0" width="1200" cellpadding="0" cellspacing="3" bgcolor="#EFECE5" align="center">
	<tr bgcolor="#FFFFFF">
		<td>		
			<form name="inUpFrm"  method="post" action="/menu03/submenu0311/lifecallhistory_InsUpDel.asp" onsubmit="return fn_inup(this);" style="margin:0">
			...
			<table width="100%" border="0" cellspacing="1" cellpadding="1" align="center" bgcolor="#ffffff">
			    <tr height="30">
					...
					<% if sGotoURL <> "" then %></td><td align='right'><a href="##" onclick="javascript:fnGoto3012('<%=sGotoURL%>');">상담관리 화면으로 이동</a></td><%else%></td><%end if%>
				</tr>
			</table>
			<table width="100%" border="0" cellspacing="1" cellpadding="1" align="center" bgcolor="#CCCCCC">
			    ...
			    <tr>
					<td bgcolor="#EEF6FF" class="TDCont" width=150 align="center"  colspan='2'>원상담</td>
					...
					<td bgcolor="#EEF6FF" class="TDCont" width=150 align="center">원상담자</td>
					<td bgcolor="#FFFFFF" width=200><input type="hidden" name="REFCNT" value="<%=REFCNT%>" readonly size='2' style="border-width:1px ; border-style:solid; text-align:right; text-forecolor=#0000ff"><input type="text" name="OriginalCustnm" value="<%=OriginalCustnm%>" readonly size='20' style="border-width:1px ; border-style:solid; text-align:left; text-forecolor=#0000ff">
					</td>	
				...
				<tr id="divREQUESTERGB" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=150 align="center" colspan='2'>원상담자와의관계</td>
...
<%
						...
						j = 0
							...
							SqlCode = SqlCode& " WHERE USEYN='Y' AND CODEGROUP='C14'"
...
-->
				<tr id="divCALLKIND_1" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=150 align="center" colspan='2'>후속확인#1&nbsp;&nbsp;<img src="/Images/Btn/BtnAdd1.gif" style="cursor:hand;" align="absmiddle" onClick="fn_AddForm('ON','6','6');"></td>
...
<%
						...
						j = 0
							'======= 처리구분 코드 가져오기 ==================================================
							SqlCode = "SELECT  CODE, CODENAME FROM tb_code"
							SqlCode = SqlCode& " WHERE codegroup = 'C13' and useyn = 'Y'"
							SqlCode = SqlCode& " ORDER BY code"
							set RsCode = db.execute(SqlCode)
				...
				<tr id="divCALLCLASS_B_6" style="display:none;">
					<td bgcolor="#FDE6F3" class="TDCont" width=150 align="center" colspan='2'>후속확인#2&nbsp;&nbsp;<img src="/Images/Btn/BtnDel1.gif" style="cursor:hand;" align="absmiddle" onClick="fn_AddForm('OFF','6','6');"></td>
...
<%
						...
						j = 0
							'======= 처리구분 코드 가져오기 ==================================================
							SqlCode = "SELECT CODE, CODENAME FROM TB_CODE"
							SqlCode = SqlCode& " WHERE USEYN='Y' AND CODEGROUP='C09'"
							SqlCode = SqlCode& " ORDER BY CODE"
							set RsCode = db.execute(SqlCode)
...
//				sms = window.open("sms.asp","sms","toolbar=no,status=yes,location=no,width=620,height=500,top=0,left=0,scrollbars=yes,resizable=no");
	...
	function fn_CustSearch(){
		//같은 고객이 있는지를 찾는다.
		//이름,cid,전화번호1,2
		ShowPOPLayer("/Include/PopUp/MemSearch.asp?FRM=after&JUBSEQ=<%=JUBSEQ%>&CB_SEQ=<%=CB_SEQ%>&SENDPHONE="+eval("inUpFrm.CID").value+"&NAME="+eval("inUpFrm.CUSTNAME").value,'800','430');		
	}

	function fn_list(){location.href="/menu03/submenu0311/lifecallhistory.asp?<%=where2%>";}
	...
	function fn_inup()
	{
		...
		if ( inUpFrm.OriginalCustnm.value.length == 0 )
		{
			alert('원상담자를 선택하십시오!');
			inUpFrm.OriginalCustnm.focus();
			return false;
		}
/*		
...
*/
	...
	function ReferUp(ty,cn){
		...
		//if (strTemp!=""){fileCNT="2"} else {fileCNT="1"}
		fileCNT="1";
		POPLayerURL = "lifecallmanage_refersearch.asp?QueryYN=Y&whereCD8="+document.all.OriginalCustnm.value;
		ShowPOPLayer(POPLayerURL,'1000','580');
	}
	...
	{
		if ( document.all.CHANNELGB_B.value == 'Q05' || document.all.CHANNELGB_B.value == 'Q07' )
		{
			// 값을 선택할 수 없다.
