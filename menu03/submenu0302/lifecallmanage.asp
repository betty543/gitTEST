...
'response.write SS_Login_Secgroup 
'response.write SS_Login_Grade 
if InType = "RECORD" then
	...
	sGotoURL = "/menu03/submenu0312/lifecallmanage.asp?InType=RECORD&RecDate="&RecDate&"&LINEKIND="&LINEKIND&"&telNo="&sCID&"&CallTIME="&CallTIME&"&FileName="&RecFileName
...
elseif InType = "CALL" then	'인입콜임.
	...
	sGotoURL = "/menu03/submenu0312/lifecallmanage.asp?InType=CALL&LINEKIND="&LINEKIND&"&telNo="&sCID &"$inroot="&CHANNELGB 
	'---------------------------------------
	'번호와 일치하는 고객있는지 찾기
	'---------------------------------------
	...
	whereCD2_B = Trim(request("whereCD2_B")) '상담방법
	...
	whereCD14 = Trim(request("whereCD14"))	'처리결과
	whereCD14_B = Trim(request("whereCD14_B"))	'처리결과
	...
	where1 = where1 & "&whereCD6_A=" & whereCD6_A& "&whereCD6_B=" & whereCD6_B& "&whereCD6_C=" & whereCD6_C& "&whereCD13=" & whereCD13 & "&whereGB=" & whereGB
	where1 = where1 & "&whereCD2_B=" & whereCD2_B& "&whereCD13_B=" & whereCD13_B& "&whereCD14=" & whereCD14& "&whereCD14_B=" & whereCD14_B
...
if JUBSEQ = "" then
	...
	if rs.eof = false then
		...
		PROCESSGB_B = rs("PROCESSGB_B")
		PROCESSGB_C = rs("PROCESSGB_C")
		WEATHER = rs("WEATHER")
		CALLID = rs("CALLID")
	...
	sSelectSql = "select * from dbo.TB_LIFECALL_DUP	where JUBSEQ = '" & JUBSEQ & "'"

	Set Rs = server.createObject("ADODB.Recordset")
	Rs.open sSelectSql,db

	do until Rs.eof
		
		if Rs("LINEGB") = "2" then
			div_2 =  "ON"
			CALLCLASS_B_2 =  Rs("BCLASS")
			CALLCLASS_C_2 =  Rs("CCLASS")
		end if

		if Rs("LINEGB") = "3" then
			div_3 =  "ON"
			CALLCLASS_B_3 =  Rs("BCLASS")
			CALLCLASS_C_3 =  Rs("CCLASS")
		end if
		
		if Rs("LINEGB") = "4" then
			div_4 =  "ON"
			CALLCLASS_B_4 =  Rs("BCLASS")
			CALLCLASS_C_4 =  Rs("CCLASS")
		end if

		if Rs("LINEGB") = "5" then
			div_5 =  "ON"
			CALLCLASS_B_5 =  Rs("BCLASS")
			CALLCLASS_C_5 =  Rs("CCLASS")
		end if

		if Rs("LINEGB") = "6" then
			div_6 =  "ON"
			CALLKIND_B_2 =  Rs("BCLASS")
			CALLKIND_C_2 =  Rs("CCLASS")
		end if
...
if JUBSEQ = "" then
	if CUSTNO = "" then
		...
		sql = " select count(*) + 1 from tb_lifecallhistory where CUSTNO = '" & CUSTNO & "'"
...
<table border="0" width="1200" cellpadding="0" cellspacing="3" bgcolor="#EFECE5" align="center">
	<tr bgcolor="#FFFFFF">
		<td>		
			...
			<input type="hidden" name="whereCD2_B" value="<%=whereCD2_B%>">
			...
			<input type="hidden" name="whereCD13" value="<%=whereCD13%>">
			<input type="hidden" name="whereCD13_B" value="<%=whereCD13_B%>">
			<input type="hidden" name="whereCD14" value="<%=whereCD14%>">
			<input type="hidden" name="whereCD14_B" value="<%=whereCD14_B%>">
			...
			<input type="text" name="PROCESSGB_B" value="<%=PROCESSGB_B%>">	
			<input type="text" name="PROCESSGB_C" value="<%=PROCESSGB_C%>">			
			...
			<table width="100%" border="0" cellspacing="1" cellpadding="1" align="center" bgcolor="#CCCCCC">
			    ...
			    <tr>
					...
					<td bgcolor="#FDE6F3" class="TDCont" width=150 align="center" colspan='2'>전화종류</td>
					<td bgcolor="#FFFFFF" colspan='5'><input type="radio" name="CHANNELGB" value="0179" class="none" <% if CHANNELGB = "0179" or CHANNELGB = "" then %>checked<%end if%> >080생명
						<input type="radio" name="CHANNELGB" value="13031" class="none" <% if CHANNELGB = "13031" then %>checked<%end if%> >1303생명
						<input type="radio" name="CHANNELGB" value="13032" class="none" <% if CHANNELGB = "13032" then %>checked<%end if%> >1303성범죄
						<input type="radio" name="CHANNELGB" value="13033" class="none" <% if CHANNELGB = "13033" then %>checked<%end if%> >1303군범죄
						<!--<input type="radio" name="CHANNELGB" value="CYBER" class="none" <% if CHANNELGB = "CYBER" OR CHANNELGB = "" then %>checked<%end if%> >사이버-->
					</td>	
				...
				<tr>
					...
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_channelgb_B&CLASSNM=CHANNELGB&CLASSGB=B&ACLASS=Q&BCLASS=<%=CHANNELGB_B%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_channelgb_B"></iframe>		
					</td>
				</tr>
				<tr>
					<td bgcolor="#EEF6FF" class="TDCont" width=50 align="center">2차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_channelgb_C&CLASSNM=CHANNELGB&CLASSGB=C&ACLASS=Q&BCLASS=<%=CHANNELGB_B%>&CCLASS=<%=CHANNELGB_C%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_channelgb_C"></iframe>	
					</td>
				</tr>
				...
				<tr id="divSOSOKGB_A" style="display:block;">
					...
					<td bgcolor="#EEF6FF" class="TDCont" align="center" width=50>1차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_A&CLASSNM=SOSOK&CLASSGB=A&ACLASS=<%=SOSOKGB_A%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_sosok_A"></iframe>		

					</td>
				</tr>
				<tr id="divSOSOKGB_B" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=50 align="center">2차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_B&CLASSNM=SOSOK&CLASSGB=B&ACLASS=<%=SOSOKGB_A%>&BCLASS=<%=SOSOKGB_B%>" scrolling="no" frameborder="0" width=100% height=50 name="frame_sosok_B"></iframe>	
					</td>
				</tr>
				<tr id="divSOSOKGB_C" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=50 align="center">3차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_C&CLASSNM=SOSOK&CLASSGB=C&ACLASS=<%=SOSOKGB_A%>&BCLASS=<%=SOSOKGB_B%>&CCLASS=<%=SOSOKGB_C%>" scrolling="no" frameborder="0" width=100% height=50 name="frame_sosok_C"></iframe>	
					</td>
				</tr>
				<tr id="divSOSOKGB_D" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=50 align="center">4차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_D&CLASSNM=SOSOK&CLASSGB=D&ACLASS=<%=SOSOKGB_A%>&BCLASS=<%=SOSOKGB_B%>&CCLASS=<%=SOSOKGB_C%>&DCLASS=<%=SOSOKGB_D%>" scrolling="no" frameborder="0" width=100% height=50 name="frame_sosok_D"></iframe>	
					</td>
				</tr>
				<tr id="divSOSOKGB_E" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=50 align="center">5차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_E&CLASSNM=SOSOK&CLASSGB=E&ACLASS=<%=SOSOKGB_A%>&BCLASS=<%=SOSOKGB_B%>&CCLASS=<%=SOSOKGB_C%>&DCLASS=<%=SOSOKGB_D%>&ECLASS=<%=SOSOKGB_E%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_sosok_E"></iframe>	
					</td>
				</tr>
				...
				<tr id="divLEVEL_B" style="display:block;">
					...
					<td bgcolor="#EEF6FF" class="TDCont" align="center" width=50>1차&nbsp;</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_level_B&CLASSNM=LEVEL&CLASSGB=B&ACLASS=P&BCLASS=<%=LEVEL_B%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_level_B"></iframe>		
					</td>
				</tr>
				<tr id="divLEVEL_C" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=50 align="center">2차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_level_C&CLASSNM=LEVEL&CLASSGB=C&ACLASS=P&BCLASS=<%=LEVEL_B%>&CCLASS=<%=LEVEL_C%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_level_C"></iframe>	
					</td>
				</tr>
				<tr id="divLEVEL_D" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=50 align="center">3차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_level_D&CLASSNM=LEVEL&CLASSGB=D&ACLASS=P&BCLASS=<%=LEVEL_B%>&CCLASS=<%=LEVEL_C%>&DCLASS=<%=LEVEL_D%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_level_D"></iframe>	
					</td>
				</tr>
...
<%
				...
				<tr id="divFAMILYGB" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=150 align="center" colspan='2'>가족사항</td>
					<td bgcolor="#FFFFFF" height=20 nowrap colspan='5'>
<%
						sReplyHtml = ""
						j = 0
							'======= 처리구분 코드 가져오기 ==================================================
							SqlCode = "SELECT CODE, CODENAME FROM TB_CODE"
							SqlCode = SqlCode& " WHERE USEYN='Y' AND CODEGROUP='C12'"
							SqlCode = SqlCode& " ORDER BY CODE"
							set RsCode = db.execute(SqlCode)

							do until RsCode.eof
								j = j + 1
								if FAMILYGB = RsCode("CODE") then
									SelectedValue = "checked"
								else
									SelectedValue = ""
								end if
								if j = 1 then
									sReplyHtml = "<input type='RADIO' value='" & RsCode("CODE") & "' name='FAMILYGB' class='none' " & SelectedValue & " >" & RsCode("CODENAME")	
								else
									sReplyHtml = sReplyHtml & "&nbsp;<input type='RADIO' value='" & RsCode("CODE") & "' name='FAMILYGB' class='none' " & SelectedValue & ">" & RsCode("CODENAME")	
								end if
								RsCode.movenext
							loop
							RsCode.close
							...
							if sReplyHtml <> "" then
								...
								sReplyHtml = "&nbsp;<img src='/Images/Comm/IconDel2.gif' title='선택취소' style='cursor:hand;' align='absmiddle' onclick=""javascript:fn_DEL('FAMILYGB');"">"
								response.write sReplyHtml
				...
				<tr id="divCALLCLASS_B_1" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=100 align="center" rowspan='2'>상담분야#1<br><img src="/Images/Btn/BtnAdd1.gif" style="cursor:hand;" align="absmiddle" onClick="fn_AddForm('ON','2','2');"></td>
					<td bgcolor="#EEF6FF" class="TDCont" align="center" width=50>1차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_B&CLASSNM=CALLCLASS&CLASSGB=B&ACLASS=O&BCLASS=<%=CALLCLASS_B%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_callclass_B"></iframe>		
					</td>
				</tr>
				<tr id="divCALLCLASS_C_1" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=50 align="center">2차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_C&CLASSNM=CALLCLASS&CLASSGB=C&ACLASS=O&BCLASS=<%=CALLCLASS_B%>&CCLASS=<%=CALLCLASS_C%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_callclass_C"></iframe>	
					</td>
				</tr>

				<tr id="divCALLCLASS_B_2" style="display:none;">
					<td bgcolor="#FDE6F3" class="TDCont" width=100 align="center" rowspan='2'>상담분야#2<br><img src="/Images/Btn/BtnAdd1.gif" style="cursor:hand;" align="absmiddle" onClick="fn_AddForm('ON','3','3');">&nbsp;<img src="/Images/Btn/BtnDel1.gif" style="cursor:hand;" align="absmiddle" onClick="fn_AddForm('OFF','2','2');"></td>
					<td bgcolor="#FDE6F3" class="TDCont" align="center" width=50>1차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_B_2&CLASSNM=CALLCLASS_2&CLASSGB=B&ACLASS=O&BCLASS=<%=CALLCLASS_B_2%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_callclass_B_2"></iframe>		
					</td>
				</tr>
				<tr id="divCALLCLASS_C_2" style="display:none;">
					<td bgcolor="#FDE6F3" class="TDCont" width=50 align="center">2차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_C_2&CLASSNM=CALLCLASS_2&CLASSGB=C&ACLASS=O&BCLASS=<%=CALLCLASS_B_2%>&CCLASS=<%=CALLCLASS_C_2%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_callclass_C_2"></iframe>	
					</td>
				</tr>

				<tr id="divCALLCLASS_B_3" style="display:none;">
					<td bgcolor="#EEF6FF" class="TDCont" width=100 align="center" rowspan='2'>상담분야#3<br><img src="/Images/Btn/BtnAdd1.gif" style="cursor:hand;" align="absmiddle" onClick="fn_AddForm('ON','4','4');">&nbsp;<img src="/Images/Btn/BtnDel1.gif" style="cursor:hand;" align="absmiddle" onClick="fn_AddForm('OFF','3','3');"></td>
					<td bgcolor="#EEF6FF" class="TDCont" align="center" width=50>1차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_B_3&CLASSNM=CALLCLASS_3&CLASSGB=B&ACLASS=O&BCLASS=<%=CALLCLASS_B_3%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_callclass_B_3"></iframe>		
					</td>
				</tr>
				<tr id="divCALLCLASS_C_3" style="display:none;">
					<td bgcolor="#EEF6FF" class="TDCont" width=50 align="center">2차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_C_3&CLASSNM=CALLCLASS_3&CLASSGB=C&ACLASS=O&BCLASS=<%=CALLCLASS_B_3%>&CCLASS=<%=CALLCLASS_C_3%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_callclass_C_3"></iframe>	
					</td>
				</tr>


				<tr id="divCALLCLASS_B_4" style="display:none;">
					<td bgcolor="#FDE6F3" class="TDCont" width=100 align="center" rowspan='2'>상담분야#4<br><img src="/Images/Btn/BtnAdd1.gif" style="cursor:hand;" align="absmiddle" onClick="fn_AddForm('ON','5','5');">&nbsp;<img src="/Images/Btn/BtnDel1.gif" style="cursor:hand;" align="absmiddle" onClick="fn_AddForm('OFF','4','4');"></td>
					<td bgcolor="#FDE6F3" class="TDCont" align="center" width=50>1차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_B_4&CLASSNM=CALLCLASS_4&CLASSGB=B&ACLASS=O&BCLASS=<%=CALLCLASS_B_4%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_callclass_B_4"></iframe>		
					</td>
				</tr>
				<tr id="divCALLCLASS_C_4" style="display:none;">
					<td bgcolor="#FDE6F3" class="TDCont" width=50 align="center">2차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_C_4&CLASSNM=CALLCLASS_4&CLASSGB=C&ACLASS=O&BCLASS=<%=CALLCLASS_B_4%>&CCLASS=<%=CALLCLASS_C_4%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_callclass_C_4"></iframe>	
					</td>
				</tr>

				<tr id="divCALLCLASS_B_5" style="display:none;">
					<td bgcolor="#EEF6FF" class="TDCont" width=100 align="center" rowspan='2'>상담분야#5<br><img src="/Images/Btn/BtnDel1.gif" style="cursor:hand;" align="absmiddle" onClick="fn_AddForm('OFF','5','5');"></td>
					<td bgcolor="#EEF6FF" class="TDCont" align="center" width=50>1차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_B_5&CLASSNM=CALLCLASS_5&CLASSGB=B&ACLASS=O&BCLASS=<%=CALLCLASS_B_5%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_callclass_B_5"></iframe>		
					</td>
				</tr>
				<tr id="divCALLCLASS_C_5" style="display:none;">
					<td bgcolor="#EEF6FF" class="TDCont" width=50 align="center">2차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_C_5&CLASSNM=CALLCLASS_5&CLASSGB=C&ACLASS=O&BCLASS=<%=CALLCLASS_B_5%>&CCLASS=<%=CALLCLASS_C_5%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_callclass_C_5"></iframe>	
					</td>
				</tr>

				<tr id="divCALLKIND_1" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=150 align="center" colspan='2'>원인제공자#1&nbsp;&nbsp;<img src="/Images/Btn/BtnAdd1.gif" style="cursor:hand;" align="absmiddle" onClick="fn_AddForm1('ON','6','6');"></td>
...
<%
						...
						j = 0
							...
							SqlCode = SqlCode& " WHERE aclass = 'R' and Bclass is not null and cclass is null"
				...
				<tr id="divCALLCLASS_B_6" style="display:none;">
					<td bgcolor="#FDE6F3" class="TDCont" width=150 align="center" colspan='2'>원인제공자#2&nbsp;&nbsp;<img src="/Images/Btn/BtnDel1.gif" style="cursor:hand;" align="absmiddle" onClick="fn_AddForm1('OFF','6','6');"></td>
...
<%
						...
						j = 0
							...
							do until RsCode.eof
								j = j + 1
								if CALLKIND_B_2 = RsCode("CODE") then
									SelectedValue = "checked"
								else
									SelectedValue = ""
								end if
								if j = 1 then
									sReplyHtml = "<input type='RADIO' value='" & RsCode("CODE") & "' name='CALLKIND_B_2' class='none' " & SelectedValue & " >" & RsCode("CODENAME")	
								else
									sReplyHtml = sReplyHtml & "&nbsp;<input type='RADIO' value='" & RsCode("CODE") & "' name='CALLKIND_B_2' class='none' " & SelectedValue & ">" & RsCode("CODENAME")	
								end if
								RsCode.movenext
							loop
							RsCode.close
							...
							if sReplyHtml <> "" then
								...
								sReplyHtml = "&nbsp;<img src='/Images/Comm/IconDel2.gif' title='선택취소' style='cursor:hand;' align='absmiddle' onclick=""javascript:fn_DEL('CALLKIND_B_2');"">"
								response.write sReplyHtml
				...
				<!--<tr id="divCALLCLASS_C_6" style="display:none;">
					<td bgcolor="#FDE6F3" class="TDCont" width=50 align="center">2차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callkind_C_2&CLASSNM=CALLKIND_2&CLASSGB=C&ACLASS=R&BCLASS=<%=CALLKIND_B_2%>&CCLASS=<%=CALLKIND_C_2%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_callkind_C_2"></iframe>	
					</td>
				</tr>-->
...
<%
				...
				<tr id="divPROCESSGB_B" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=100 align="center" rowspan='2'>조치결과</td>
					<td bgcolor="#EEF6FF" class="TDCont" align="center" width=50>1차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_processgb_B&CLASSNM=PROCESSGB&CLASSGB=B&ACLASS=U&BCLASS=<%=PROCESSGB_B%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_processgb_B"></iframe>		
					</td>
				</tr>
				<tr id="divPROCESSGB_C" style="display:block;">
					<td bgcolor="#EEF6FF" class="TDCont" width=50 align="center">2차</td>
					<td bgcolor="#FFFFFF" nowrap colspan='5'><iframe src="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_processgb_C&CLASSNM=PROCESSGB&CLASSGB=C&ACLASS=U&BCLASS=<%=PROCESSGB_B%>&CCLASS=<%=PROCESSGB_C%>" scrolling="no" frameborder="0" width=100% height=25 name="frame_processgb_C"></iframe>	
					</td>
				</tr>

					<td bgcolor="#EEF6FF" class="TDCont" width=150 align="center" colspan='2'>날  씨</td>
					<td bgcolor="#FFFFFF" height=20 nowrap colspan='5'>
<%
						sReplyHtml = ""
						j = 0
							'======= 처리구분 코드 가져오기 ==================================================
							SqlCode = "SELECT CODE, CODENAME FROM TB_CODE"
							SqlCode = SqlCode& " WHERE USEYN='Y' AND CODEGROUP='C11'"
							SqlCode = SqlCode& " ORDER BY CODE"
							set RsCode = db.execute(SqlCode)

							do until RsCode.eof
								j = j + 1
								if WEATHER = RsCode("CODE") then
									SelectedValue = "checked"
								else
									SelectedValue = ""
								end if
								if j = 1 then
									sReplyHtml = "<input type='RADIO' value='" & RsCode("CODE") & "' name='WEATHER' class='none' " & SelectedValue & " >" & RsCode("CODENAME")	
								else
									sReplyHtml = sReplyHtml & "&nbsp;<input type='RADIO' value='" & RsCode("CODE") & "' name='WEATHER' class='none' " & SelectedValue & ">" & RsCode("CODENAME")	
								end if
								RsCode.movenext
							loop
							RsCode.close
							...
							if sReplyHtml <> "" then
								...
								sReplyHtml = "&nbsp;<img src='/Images/Comm/IconDel2.gif' title='선택취소' style='cursor:hand;' align='absmiddle' onclick=""javascript:fn_DEL('WEATHER');"">"
								response.write sReplyHtml
...
//				sms = window.open("sms.asp","sms","toolbar=no,status=yes,location=no,width=620,height=500,top=0,left=0,scrollbars=yes,resizable=no");
	...
	{
		frame_level.location = "/menu03/submenu0301/frame_level.asp?level="+arg+"&level2=";
	}
	function fn_SetSosok2(arg)
	{
		frame_sosok.location = "/menu03/submenu0301/frame_sosok.asp?SOSOKGB="+arg+"&SOSOKETCGB=";
		frame_sosok2.location = "/menu03/submenu0301/frame_sosok_3.asp?SOSOKGB="+arg+"&SOSOKETCGB=";
...
*/
	...
	{
		//alert(arg0);
		if ( arg0 == 'WEATHER' || arg0 == 'REQUESTERGB' || arg0 == 'FAMILYGB' || arg0 == 'CALLFLAG' || arg0 == 'CALLKIND_B' || arg0 == 'CALLKIND_B_2')
		{
			//날씨
//alert(1);
			for (var i=0;i<=100;i++)
			{
				//alert(eval("inUpFrm."+arg0+"["+i+"]"));
		...
		else if ( arg0 == 'frame_callkind_B' ) //원인제공자#1-1차
		{
			...
			frame_callkind_B.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callkind_B&CLASSNM=CALLKIND&CLASSGB=B&ACLASS=R&BCLASS=";
			frame_callkind_C.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_C&CLASSNM=CALLKIND&CLASSGB=C&ACLASS=O&BCLASS=&CCLASS=";
		}
		else if ( arg0 == 'frame_callkind_C' ) //원인제공자#1-2차
		{
			
			document.all.CALLKIND_C.value = "";
			frame_callkind_C.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_C&CLASSNM=CALLKIND&CLASSGB=C&ACLASS=O&BCLASS="+document.all.CALLKIND_B.value+"&CCLASS=";
		}
		else if ( arg0 == 'frame_callkind_B_2' ) //원인제공자#1-1차
		{
			document.all.CALLKIND_B_2.value = "";
			document.all.CALLKIND_C_2.value = "";
			frame_callkind_B_2.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callkind_B_2&CLASSNM=CALLKIND_2&CLASSGB=B&ACLASS=R&BCLASS=";
			frame_callkind_C_2.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callkind_C_2&CLASSNM=CALLKIND_2&CLASSGB=C&ACLASS=O&BCLASS=&CCLASS=";
		}
		else if ( arg0 == 'frame_callkind_C_2' ) //원인제공자#1-2차
		{
			document.all.CALLKIND_C_2.value = "";
			frame_callkind_C_2.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callkind_C_2&CLASSNM=CALLKIND_2&CLASSGB=C&ACLASS=O&BCLASS="+document.all.CALLKIND_B_2.value+"&CCLASS=";
		}
		...
		else if ( arg0 == 'frame_callclass_B' ) //상담분야#1-1차
		{
			...
			frame_callclass_B.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_B&CLASSNM=CALLCLASS&CLASSGB=B&ACLASS=O&BCLASS=";
			frame_callclass_C.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_C&CLASSNM=CALLCLASS&CLASSGB=C&ACLASS=O&BCLASS=&CCLASS=";
		}
		else if ( arg0 == 'frame_callclass_C' ) //상담분야#1-2차
		{
			document.all.CALLCLASS_C.value = "";
			frame_callclass_C.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_C&CLASSNM=CALLCLASS&CLASSGB=C&ACLASS=O&BCLASS="+document.all.CALLCLASS_B.value+"&CCLASS=";
		}
		else if ( arg0 == 'frame_callclass_B_2' ) //상담분야#2-1차
		{
			document.all.CALLCLASS_B_2.value = "";
			document.all.CALLCLASS_C_2.value = "";
			frame_callclass_B_2.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_B_2&CLASSNM=CALLCLASS_2&CLASSGB=B&ACLASS=O&BCLASS=";
			frame_callclass_C_2.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_C_2&CLASSNM=CALLCLASS_2&CLASSGB=C&ACLASS=O&BCLASS=&CCLASS=";
		}
		else if ( arg0 == 'frame_callclass_C_2' ) //상담분야#1-2차
		{
			document.all.CALLCLASS_C_2.value = "";
			frame_callclass_C_2.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_callclass_C_2&CLASSNM=CALLCLASS_2&CLASSGB=C&ACLASS=O&BCLASS="+document.all.CALLCLASS_B_2.value+"&CCLASS=";
		}
		else if ( arg0 == 'frame_channelgb_B' ) //상담유형
		{
			document.all.CHANNELGB_B.value = "";
			document.all.CHANNELGB_C.value = "";
			frame_channelgb_B.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_channelgb_B&CLASSNM=CHANNELGB&CLASSGB=B&ACLASS=Q&BCLASS=";
			frame_channelgb_C.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_channelgb_C&CLASSNM=CHANNELGB&CLASSGB=C&ACLASS=Q&BCLASS=&CCLASS=";
		}
		else if ( arg0 == 'frame_channelgb_C' ) //상담유형
		{
			document.all.CHANNELGB_C.value = "";
			frame_channelgb_C.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_channelgb_C&CLASSNM=CHANNELGB&CLASSGB=C&ACLASS=Q&BCLASS="+document.all.CHANNELGB_B.value+"&CCLASS=";
		}
		else if ( arg0 == 'frame_level_B' ) //계급1차
		{
			...
			frame_level_B.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_level_B&CLASSNM=LEVEL&CLASSGB=B&ACLASS=P&BCLASS=";
			frame_level_C.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_level_C&CLASSNM=LEVEL&CLASSGB=C&ACLASS=P&BCLASS=&CCLASS=&DCLASS=";
			frame_level_D.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_level_C&CLASSNM=LEVEL&CLASSGB=C&ACLASS=P&BCLASS=&CCLASS=&DCLASS=";
		}
		else if ( arg0 == 'frame_level_C' ) //계급1차
		{
			document.all.LEVEL_C.value = "";
			document.all.LEVEL_D.value = "";
			frame_level_C.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_level_C&CLASSNM=LEVEL&CLASSGB=C&ACLASS=P&BCLASS="+document.all.LEVEL_B.value+"&CCLASS=";
			frame_level_D.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_level_D&CLASSNM=LEVEL&CLASSGB=D&ACLASS=P&BCLASS="+document.all.LEVEL_B.value+"&CCLASS=&DCLASS=";
		}
		else if ( arg0 == 'frame_level_D' ) //계급1차
		{
			document.all.LEVEL_D.value = "";
			frame_level_D.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_level_D&CLASSNM=LEVEL&CLASSGB=D&ACLASS=P&BCLASS="+document.all.LEVEL_B.value+"&CCLASS="+document.all.LEVEL_C.value+"&DCLASS=";
		}
		else if ( arg0 == 'frame_sosok_A' )
		{
			...
			frame_sosok_A.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_A&CLASSNM=SOSOK&CLASSGB=A&ACLASS=";
			frame_sosok_B.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_B&CLASSNM=SOSOK&CLASSGB=B&ACLASS=&BCLASS=";
			frame_sosok_C.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_C&CLASSNM=SOSOK&CLASSGB=C&ACLASS=&BCLASS=&CCLASS=";
			frame_sosok_D.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_D&CLASSNM=SOSOK&CLASSGB=D&ACLASS=&BCLASS=&CCLASS=&DCLASS=";
			frame_sosok_E.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_E&CLASSNM=SOSOK&CLASSGB=E&ACLASS=&BCLASS=&CCLASS=&DCLASS=&ECLASS=";
		}
		else if ( arg0 == 'frame_sosok_B' )
		{
			...
			document.all.SOSOKGB_D.value = "";
			document.all.SOSOKGB_E.value = "";
			frame_sosok_B.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_B&CLASSNM=SOSOK&CLASSGB=B&ACLASS="+document.all.SOSOKGB_A.value + "&BCLASS=";
			frame_sosok_C.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_C&CLASSNM=SOSOK&CLASSGB=C&ACLASS="+document.all.SOSOKGB_A.value + "&BCLASS=&CCLASS=";
			frame_sosok_D.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_D&CLASSNM=SOSOK&CLASSGB=D&ACLASS="+document.all.SOSOKGB_A.value + "&BCLASS=&CCLASS=&DCLASS=";
			frame_sosok_E.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_E&CLASSNM=SOSOK&CLASSGB=E&ACLASS="+document.all.SOSOKGB_A.value + "&BCLASS=&CCLASS=&DCLASS=&ECLASS=";
		}
		else if ( arg0 == 'frame_sosok_C' )
		{
			...
			document.all.SOSOKGB_D.value = "";
			document.all.SOSOKGB_E.value = "";
			frame_sosok_C.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_C&CLASSNM=SOSOK&CLASSGB=C&ACLASS="+document.all.SOSOKGB_A.value + "&BCLASS="+document.all.SOSOKGB_B.value + "&CCLASS=";
			frame_sosok_D.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_D&CLASSNM=SOSOK&CLASSGB=D&ACLASS="+document.all.SOSOKGB_A.value + "&BCLASS="+document.all.SOSOKGB_B.value + "&CCLASS=&DCLASS=";
			frame_sosok_E.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_E&CLASSNM=SOSOK&CLASSGB=E&ACLASS="+document.all.SOSOKGB_A.value + "&BCLASS="+document.all.SOSOKGB_B.value + "&CCLASS=&DCLASS=&ECLASS=";
		}
		else if ( arg0 == 'frame_sosok_D' )
		{
			document.all.SOSOKGB_D.value = "";
			document.all.SOSOKGB_E.value = "";
			frame_sosok_D.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_D&CLASSNM=SOSOK&CLASSGB=D&ACLASS="+document.all.SOSOKGB_A.value + "&BCLASS="+document.all.SOSOKGB_B.value + "&CCLASS="+document.all.SOSOKGB_C.value + "&DCLASS=";
			frame_sosok_E.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_E&CLASSNM=SOSOK&CLASSGB=E&ACLASS="+document.all.SOSOKGB_A.value + "&BCLASS="+document.all.SOSOKGB_B.value + "&CCLASS="+document.all.SOSOKGB_C.value + "&DCLASS=&ECLASS=";
		}
		else if ( arg0 == 'frame_sosok_E' )
		{
			document.all.SOSOKGB_E.value = "";
			frame_sosok_E.location.href = "/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_sosok_E&CLASSNM=SOSOK&CLASSGB=E&ACLASS="+document.all.SOSOKGB_A.value + "&BCLASS="+document.all.SOSOKGB_B.value + "&CCLASS="+document.all.SOSOKGB_C.value + "&DCLASS="+document.all.SOSOKGB_D.value + "&ECLASS=";
		}
		else if ( arg0 == 'frame_processgb_B' ) //조치결과
		{
			document.all.PROCESSGB_B.value = "";
			document.all.PROCESSGB_C.value = "";
			frame_processgb_B.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_processgb_B&CLASSNM=PROCESSGB&CLASSGB=B&ACLASS=U&BCLASS=";
			frame_processgb_C.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_processgb_C&CLASSNM=PROCESSGB&CLASSGB=C&ACLASS=U&BCLASS=&CCLASS=";
		}
		else if ( arg0 == 'frame_processgb_C' ) //조치결과
		{
			document.all.PROCESSGB_C.value = "";
			frame_processgb_C.location.href ="/menu03/submenu0301/frame_CLASS.asp?frame_nm=frame_processgb_C&CLASSNM=PROCESSGB&CLASSGB=C&ACLASS=U&BCLASS="+document.all.PROCESSGB_B.value+"&CCLASS=";
		}
	}
	...
	function fn_chkCHANNELGB_B()
	{
		if ( document.all.CHANNELGB_B.value == 'Q05' || document.all.CHANNELGB_B.value == 'Q07' || document.all.CHANNELGB_B.value == 'Q99')
		{
			...
			eval("divCALLCLASS_B_2").style.display = "none";
			eval("divCALLCLASS_C_2").style.display = "none";
			eval("divCALLCLASS_B_3").style.display = "none";
			eval("divCALLCLASS_C_3").style.display = "none";
			eval("divCALLCLASS_B_4").style.display = "none";
			eval("divCALLCLASS_C_4").style.display = "none";
			eval("divCALLCLASS_B_5").style.display = "none";
			eval("divCALLCLASS_C_5").style.display = "none";

			eval("divCALLCLASS_B_6").style.display = "none";
			//eval("divCALLCLASS_C_6").style.display = "none";

			eval("divCALLKIND_1").style.display = "none";
			//eval("divCALLKIND_2").style.display = "none";
			eval("divREQUESTERGB").style.display = "none";
			eval("divPROCESSGB_B").style.display = "none";
			eval("divPROCESSGB_C").style.display = "none";
		...
		else
		{
			...
			if ( eval("inUpFrm.div_2").value =="ON" )
			{
				eval("divCALLCLASS_B_2").style.display = "block";
				eval("divCALLCLASS_C_2").style.display = "block";
			}
			else
			{
				eval("divCALLCLASS_B_2").style.display = "none";
			...
			if ( eval("inUpFrm.div_3").value =="ON" )
			{
				eval("divCALLCLASS_B_3").style.display = "block";
				eval("divCALLCLASS_C_3").style.display = "block";
			}
			else
			{
				eval("divCALLCLASS_B_3").style.display = "none";
			...
			if ( eval("inUpFrm.div_4").value =="ON" )
			{
				eval("divCALLCLASS_B_4").style.display = "block";
				eval("divCALLCLASS_C_4").style.display = "block";
			}
			else
			{
				eval("divCALLCLASS_B_4").style.display = "none";
			...
			if ( eval("inUpFrm.div_5").value =="ON" )
			{
				eval("divCALLCLASS_B_5").style.display = "block";
				eval("divCALLCLASS_C_5").style.display = "block";
			}
			else
			{
				eval("divCALLCLASS_B_5").style.display = "none";
			...
			//eval("divCALLKIND_2").style.display = "block";
			if ( eval("inUpFrm.div_6").value =="ON" )
			{
				eval("divCALLCLASS_B_6").style.display = "block";
				//eval("divCALLCLASS_C_6").style.display = "block";
			}
			else
			{
				eval("divCALLCLASS_B_6").style.display = "none";
			...
			eval("divPROCESSGB_B").style.display = "block";				
			eval("divPROCESSGB_C").style.display = "block";				
	...
	function fn_listdel(arg0)
	{
		...
		{
			location.href="/menu03/submenu0301/lifecallhistory_InsUpDel.asp?guboon=DEL&jubseq="+arg0+"&<%=where2%>";
		}
	}
