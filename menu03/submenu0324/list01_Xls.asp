...
Call Response.AddHeader("Content-Disposition", "attachment; filename=군범죄신고현황_" &Date()& ".xls")	'바로저장하기
	...
	<tr>
		<td>
			...
			<table width="2000"  border="1" cellpadding="0" cellspacing="0" bgcolor="#EFECE5" align="left" bordercolor="black" bordercolordark="white" bordercolorlight="black">
				...
				subSql = subSql & "	 from TB_crimecallhistory  where jubdate >= '" & FromDate & "' AND		jubdate <= '" & ToDate & "'  " & chSql & " "
			...
			<table width="1200"  border="1" cellpadding="0" cellspacing="0" bgcolor="#EFECE5" align="left" bordercolor="black" bordercolordark="white" bordercolorlight="black">
				...
				'상담관별 총계
				SQL = "select * from ( "
				SQL = SQL & " 	SELECT	'1' gubun, count(incode) cnt FROM	TB_crimecallhistory	where jubdate >= '" & FromDate & "'"
				SQL = SQL & "		AND		jubdate <= '" & ToDate & "' and datepart(weekday,jubdate) = 2  " & chSql & " "
				SQL = SQL & "	union all "
				SQL = SQL & " 	SELECT	'2' gubun, count(incode) cnt FROM	TB_crimecallhistory	where jubdate >= '" & FromDate & "'"
				SQL = SQL & "		AND		jubdate <= '" & ToDate & "' and datepart(weekday,jubdate) = 3  " & chSql & "  "
				SQL = SQL & "	union all "
				SQL = SQL & " 	SELECT	'3' gubun, count(incode) cnt  FROM	TB_crimecallhistory	where jubdate >= '" & FromDate & "'"
				SQL = SQL & "		AND		jubdate <= '" & ToDate & "' and datepart(weekday,jubdate) = 4  " & chSql & " "
				SQL = SQL & "	union all "
				SQL = SQL & " 	SELECT	'4' gubun, count(incode) cnt  FROM	TB_crimecallhistory	where jubdate >= '" & FromDate & "'"
				SQL = SQL & "		AND		jubdate <= '" & ToDate & "' and datepart(weekday,jubdate) = 5  " & chSql & " "
				SQL = SQL & "	union all "
				SQL = SQL & " 	SELECT	'5' gubun, count(incode) cnt  FROM	TB_crimecallhistory	where jubdate >= '" & FromDate & "'"
				SQL = SQL & "		AND		jubdate <= '" & ToDate & "' and datepart(weekday,jubdate) = 6  " & chSql & " "
				SQL = SQL & "	union all "
				SQL = SQL & " 	SELECT	'6' gubun, count(incode) cnt  FROM	TB_crimecallhistory	where jubdate >= '" & FromDate & "'"
				SQL = SQL & "		AND		jubdate <= '" & ToDate & "' and datepart(weekday,jubdate) = 7  " & chSql & " "
				SQL = SQL & "	union all "
				SQL = SQL & " 	SELECT	'7' gubun, count(incode) cnt  FROM	TB_crimecallhistory	where jubdate >= '" & FromDate & "'"
				SQL = SQL & "		AND		jubdate <= '" & ToDate & "' and datepart(weekday,jubdate) = 1  " & chSql & " "
				SQL = SQL & "	union all "
				SQL = SQL & " 	SELECT	'8' gubun, count(incode) cnt  FROM	TB_crimecallhistory	where jubdate >= '" & FromDate & "'"
			...
			<table width="1200"  border="1" cellpadding="0" cellspacing="0" bgcolor="#EFECE5" align="left" bordercolor="black" bordercolordark="white" bordercolorlight="black">
				<tr height="30">
					<td align="left" bgcolor="#FFFFFF" class="TDCont" colspan="12">&nbsp;&nbsp;<b><font color="#ff00ff"></font> 8. 조치별</b></td>
				</tr>
				<tr bgcolor='#EEF6FF'>
					<td align='center' class='TDCont' width='150'>구분</td>
