...
<table border="0" width="940" cellpadding="0" cellspacing="3" bgcolor="#EFECE5" align="center">
	<tr bgcolor="#FFFFFF">
		<td>		
			<form name="inUpFrm"  method="post" action="/menu03/submenu0301/lifecallhistory_InsUpDel_frame.asp" onsubmit="return fn_inup(this);" style="margin:0">
...
<%
				...
				<tr>
					...
					<td bgcolor="#FFFFFF" width=200><input type="hidden" name="REFERJUBSEQ" value="<%=REFERJUBSEQ%>">
...
<%
			    ...
			    <tr>
					...
					<td bgcolor="#EEF6FF" class="TDCont" width=100 align="center" rowspan=2>첨부파일</td>
					<td bgcolor="#FFFFFF" width=200 rowspan=2 nowrap><input type="hidden" name="FILENAME1" value="<%=FILENAME1%>" readonly>
...
//				sms = window.open("sms.asp","sms","toolbar=no,status=yes,location=no,width=620,height=500,top=0,left=0,scrollbars=yes,resizable=no");
	...
	function fn_CustSearch(){
		//같은 고객이 있는지를 찾는다.
		//이름,cid,전화번호1,2
		ShowPOPLayer("/Include/PopUp/MemSearch.asp?FRM=lifepop&JUBSEQ=<%=JUBSEQ%>&CB_SEQ=<%=CB_SEQ%>&SENDPHONE="+eval("inUpFrm.CID").value+"&NAME="+eval("inUpFrm.CUSTNAME").value,'800','430');		
	}
