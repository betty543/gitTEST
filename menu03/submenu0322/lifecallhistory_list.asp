...
%>
		...
		<tr bgcolor="#FFFFFF">
			...
			<td align="center"><%=db_GetCodeName("A14",db_CHANNELGB)%>><%=db_getCateNameB_("Q",db_CHANNELGB_B)%><%if db_getCateNameB_("Q",db_CHANNELGB_B) <> "" then %>><%end if%><%=db_getCateNameC_("Q",db_CHANNELGB_B,db_CHANNELGB_C)%></td>
			<td align="center"><%=db_getCateNameB_("S",db_CALLCLASS_B)%><%if db_getCateNameB_("S",db_CALLCLASS_B) <> "" then %>><%end if%><%=db_getCateNameC_("S",db_CALLCLASS_B,db_CALLCLASS_C)%></td>
...
<script>
	...
	function fn_print(arg0) {	
		...
		//parent.ShowPOPLayer("/menu03/submenu0301/lifecallmanage_print.asp?guboon=UP&jubseq="+arg0,'760','600');		
		var obj = window.open('/menu03/submenu0321/lifecallmanage_print.asp?guboon=UP&jubseq='+arg0,'lifecallmanage_print','top=0,left=0,width=760,height=600,scrollbars=yes');
		obj.focus();
	}
	...
	function fn_del(arg0,arg1) {	
		...
		{
			location.href="/menu03/submenu0321/lifecallhistory_InsUpDel_frame.asp?guboon=DEL&jubseq="+arg0+"&<%=where2%>&CUSTNO=<%=CUSTNO%>";
		}
	}

	function fn_update(arg0,arg1) {	

		parent.ShowPOPLayer("/menu03/submenu0322/lifecallmanage_pop.asp?guboon=UP&jubseq="+arg0+"&<%=where2%>",'960','520');	
	}
