...
SS_Login_Secgroup = SESSION("SS_Login_Secgroup")
	...
	Call Response.AddHeader("Content-Disposition", "attachment; filename=사후상담일지_" &FromDate&"_"&ToDate& ".xls")	'바로저장하기
			...
			<table width="600" height="80" border="1" cellspacing="1" cellpadding="1" align="center" bgcolor="#ffffff" bordercolor="black" bordercolordark="white" bordercolorlight="black">
			    <tr height="80">
					<td align='center' height="80" colspan='6'>
						<b><font color="#000000" size="5px" >사후상담일지</font></b>
					</td>
...
<%
	...
	if db_GetCodeName("C09",db_PROCESSGB) <> "" then
		if sFirstLine = "" then
			sFirstLine = db_GetCodeName("C09",db_PROCESSGB)
		else
			sFirstLine = sFirstLine & " - " & db_GetCodeName("C09",db_PROCESSGB)
		end if
	end if
...
<!--<body leftmargin="0" marginwidth="0" topmargin="0" marginheight="0" border="1">-->
			...
			<table width="600" border="1" cellspacing="1" cellpadding="1" align="center" bgcolor="#CCCCCC" bordercolor="black" bordercolordark="white" bordercolorlight="black">
			    <tr>
					<td bgcolor="#EEF6FF" class="TDCont" width=100 align="left" colspan='6'>[접수번호:<%=db_JUBSEQ%>]&nbsp;<%=left(db_JUBTIME1,10)%>&nbsp;<%=mid(db_JUBTIME1,12,5)%>&nbsp;~&nbsp;<%=mid(db_JUBTIME2,12,5)%>&nbsp;(<%=db_CALLTIMEDP%>)&nbsp;<%=db_getCateNameB_("Q",db_CHANNELGB_B)%><%if db_getCateNameC_("Q",db_CHANNELGB_B,db_CHANNELGB_C) <> "" then %>><%end if%><%=db_getCateNameC_("Q",db_CHANNELGB_B,db_CHANNELGB_C)%>&nbsp;[<%=sFirstLine%> 상담관]
					<br>
