...
'response.Write "INS"
	...
	SQL = "select MAX(JUBSEQ) from TB_LIFECALLHISTORY_OB where LEFT(JUBSEQ,6) = CONVERT(CHAR(6),GETDATE(),112)"
	Rs.open SQL,db
	IF ISNULL(Rs(0)) THEN
	...
	db.begintrans	
		...
		SQL = " INSERT INTO TB_LIFECALLHISTORY_OB (JUBSEQ"
		SQL = SQL & "	,	JUBDATE"
		SQL = SQL & "	,	JUBTIME"
		SQL = SQL & "	,	IOFLAG"
		SQL = SQL & "	,	EMERYN"
		...
		if flag = "Y" then
			...
			pageUrl = "/menu03/submenu0311/lifecallhistory.asp"
			Call MsgGoUrl( "정상적으로 등록되었습니다.",pageUrl)
		else
			db.RollBackTrans
			LogWrite "ERROR_SQL="&SQL, "lifecallhistory_InsUpDel.asp", "/menu03/submenu0311/"
...
case "UP"
	...
	flag = "N"			'UPDATE 성공여부
		...
		SQL = " UPDATE TB_LIFECALLHISTORY_OB SET "
...
'response.write SQL
	...
	if flag = "Y" then
		...
		pageUrl = "/menu03/submenu0311/lifecallhistory.asp?"&where2
...
case "DEL"
	...
	SQL = "DELETE FROM TB_LIFECALLHISTORY_OB WHERE JUBSEQ = '" & TRIM(JUBSEQ) & "'"
