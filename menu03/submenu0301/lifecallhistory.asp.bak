...
whereCD5_D = Trim(request("whereCD5_D")) '소속
	...
	<tr bgcolor="#FFFFFF">
		<td>
			...
			<table width="100%" border="0" cellspacing="1" cellpadding="1" align="center" bgcolor="#CCCCCC">
				...
				<td bgcolor="#EFEFEF" class="TDCont" width=80 align='center'>전화종류</td>
					<td bgcolor="#FFFFFF"  nowrap colspan='1'>&nbsp;
						<select name="whereGB" class="ComboFFFCE7">
							<option value="" selected>전화종류선택</option>
							<!--<option value="0179" <% if whereGB = "0179" then %>selected<% end if %>>080생명</option>-->
							<option value="13031" <% if whereGB = "13031" then %>selected<% end if %>>1303생명</option>
							<option value="13032" <% if whereGB = "13032" then %>selected<% end if %>>1303성범죄</option>
							<option value="13032" <% if whereGB = "13033" then %>selected<% end if %>>1303군범죄</option>
							<!--<option value="CYBER" <% if whereGB = "CYBER" then %>selected<% end if %>>사이버</option>-->
						</select>
					</td>
					<td colspan='1' rowspan="4" bgcolor="#FFFFFF" align="center"><img src="/Images/Btn/BtnSearch.gif" style="cursor:hand;" onClick="fn_Search();"></td>
				</tr>
				<tr valign="center" style="display:block;">
					...
					<td bgcolor="#FFFFFF" colspan='1'>&nbsp;
						...
						</select>&nbsp;&nbsp;키워드:&nbsp;<input value="<%=whereCD9%>" name="whereCD9" type="text" size="20" onfocus="setFocusColor(this);" onblur="setOutColor(this);">
					</td>


					<td bgcolor="#EFEFEF" class="TDCont" width=70 align='center'>조치결과</td>
					<td bgcolor="#FFFFFF" colspan=5>&nbsp;
						<iframe src="Processgb_FrameA.asp?PROCESSGB_A=<%=whereCD14%>&PROCESSGB_B=" scrolling="no" frameborder="0" border="0" name="ProcessgbFrameA" width="80"></iframe>
						<iframe src="Processgb_FrameB.asp?PROCESSGB_A=<%=whereCD14%>&PROCESSGB_B=<%=whereCD14_B%>" scrolling="no" frameborder="0" border="0" name="ProcessgbFrameB" width="80"></iframe>
					</td>	
		    ...
		    <tr valign="center" height="30">
					...
					<td bgcolor="#FFFFFF" colspan="1" >&nbsp;
						<!--CALLCLASS-->
						<iframe src="CallClass_FrameA.asp?CALLCLASS_A=<%=whereCD13%>&CALLCLASS_B=" scrolling="no" frameborder="0" border="0" name="CallClassFrameA" width="80"></iframe>
						<iframe src="CallClass_FrameB.asp?CALLCLASS_A=<%=whereCD13%>&CALLCLASS_B=<%=whereCD13_B%>" scrolling="no" frameborder="0" border="0" name="CallClassFrameB" width="80"></iframe>

					</td>

					<td bgcolor="#EFEFEF" class="TDCont" width=70 align='center'>상담유형</td>
					<td bgcolor="#FFFFFF" colspan=5>&nbsp;
						<iframe src="Channel_FrameA.asp?CHANNEL_A=<%=whereCD2%>&CHANNEL_B=" scrolling="no" frameborder="0" border="0" name="ChannelFrameA" width="80"></iframe>
						<iframe src="Channel_FrameB.asp?CHANNEL_A=<%=whereCD2%>&CHANNEL_B=<%=whereCD2_B%>" scrolling="no" frameborder="0" border="0" name="ChannelFrameB" width="80"></iframe>
					</td>					
				</tr>
...
<table border="0" width="1200" cellspacing="1" cellpadding="0" width="100%" bgcolor="#CCCCCC" align="center">
	...
	DO UNTIL RS.EOF
		...
		<tr bgcolor="#FFFFFF">
			...
			<td align="left"><%=db_getCateNameB_("Q",db_CHANNELGB_B)%><%if db_getCateNameB_("Q",db_CHANNELGB_B) <> "" then %>><%end if%><%=db_getCateNameC_("Q",db_CHANNELGB_B,db_CHANNELGB_C)%></td>
			<td align="left"><%=db_getCateNameB_("O",db_CALLCLASS_B)%><%if db_getCateNameB_("O",db_CALLCLASS_B) <> "" then %>><%end if%><%=db_getCateNameC_("O",db_CALLCLASS_B,db_CALLCLASS_C)%></td>
			<td align="left"><%=db_getCateNameB_("U",db_PROCESSGB_B)%><%if db_getCateNameB_("U",db_PROCESSGB_B) <> "" then %>><%end if%><%=db_getCateNameC_("U",db_PROCESSGB_B,db_PROCESSGB_C)%></td>
...
<script>
	...
	function fn_Detail(arg0,arg1) {
		historyDetail.location = "/menu03/submenu0301/History_Detail.asp?JUBSEQ="+arg0+"&Keyword="+arg1;
	}

	function fn_print(arg0) {
		ShowPOPLayer("/menu03/submenu0301/lifecallmanage_print.asp?guboon=UP&jubseq="+arg0,'760','600');
	}

	function fn_save(arg0) {
		location.href="/menu03/submenu0301/lifecallmanage_save.asp?guboon=UP&jubseq="+arg0;
	}
	...
	function fn_update(arg0,arg1) {
			location.href="/menu03/submenu0302/lifecallmanage.asp?guboon=UP&jubseq="+arg0+"&<%=where2%>";
	}
