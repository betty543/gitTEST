...
<%
	smsgroupname=Request("smsgroupname")
	smsgroupid=Request("smsgroupid") 
...
%>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
        		<tr><td height="22" colspan="2" class="FBlk">◈ <b> [<%=smsgroupname%>] 그룹의 주소록</b></td><td height=28 colspan="1" height="1" align="right"><img src="/Images/Btn/BtnAdd.gif" title="주소록추가" style="cursor:hand;" align="absmiddle" onClick="ShowPOPLayer('smsgroup_addressdetail.asp?group_idx=<%=smsgroupid%>','500','250');"></td></tr>
        	</table>
...
%>
				<tr onmouseover="this.style.background='#FFFCE7'" bgcolor="#FFFFFF" onmouseout="this.style.background='#FFFFFF'" >
					...
					<td align="center" width=10%><%=sname%></td>
					<td align="center" width=15%><%=cellphone%></td>
					<td align="center">
...
<%
				...
				if ( i = 0 ) then
%>
        			<tr><td colspan="7" height="100" bgcolor="#FFFFFF" align="center">선택한 그룹에 등록된 자료가 없습니다.</td></tr>
<%
				end if
%>
        	</table>    
...
<!--
	function fn_update(arg0, arg1){
		//sms그룹 수정
		ShowPOPLayer('smsgroup_addressdetail.asp?group_idx=<%=smsgroupid%>&idx='+arg0,'500','250');
	}
	function fn_del(arg0, arg1){
		//sms그룹 수정
		if (confirm("선택한 자료를 삭제하시겠습니까?"))
			ShowPOPLayer('smsgroup_addressdetail.asp?group_idx=<%=smsgroupid%>&idx='+arg0+'&JOBGB=D','500','250');
	}
