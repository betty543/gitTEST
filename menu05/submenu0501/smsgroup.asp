	...
	<tr valign="top" height="450">
		...
		<td width="340">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
        		<tr><td height="22" colspan="2" class="FBlk">◈ <b>그룹</b></td><td colspan="5" align="right" height=28><img src="/Images/Btn/BtnAdd.gif" title="그룹추가" style="cursor:hand;" align="absmiddle" onClick="ShowPOPLayer('smsgroup_detail.asp','500','210');"></td></tr>
        	</table>
...
<%
				...
				SQL = "	select * from TB_SMSGROUP where groupgb in ('2')" '공용
				SQL = SQL & " union all	select * from TB_SMSGROUP where groupgb in ('1') "
				if SS_Login_Secgroup = "A" then
					SQL = SQL & "	and	incode = '" & INCODE & "'"
				elseif SS_Login_Secgroup = "B" then
					'우리팀껏만 보이도록 한다
					SQL = SQL & "	and	incode in ( select userid from tb_userinfo where Grade = '" & SS_Login_Grade & "')"
				end if
				SQL = SQL & "	order by idx" '공용
				i = 0
				...
				do until Rs.eof
						...
						if rs("groupgb") = "1" then
							groupgb_NM = db_getUserName(rs("incode"))
						else
							groupgb_NM = "공용"
						end if
				...
				<tr onmouseover="this.style.background='#FFFCE7'" bgcolor="#FFFFFF" onmouseout="this.style.background='#FFFFFF'">
					...
					<td align="center" width="60">
							<img src="/Images/Comm/IconHome.gif" title="목록보기" style="cursor:hand;" align="absmiddle" onclick="document.all.ogroupidx.value=<%=idx%>; ListFrame.location.href='smsgroup_addresslist.asp?smsgroupid=<%=idx%>&smsgroupname=<%=groupgb_NM%>-<%=groupname%>';">
...
%>
				<!--<tr onmouseover="this.style.background='#FFFCE7'" bgcolor="#FFFFFF" onmouseout="this.style.background='#FFFFFF'">
					...
					<td align="center" width="60">
							<img src="/Images/Comm/IconHome.gif" title="목록보기" style="cursor:hand;" align="absmiddle" onclick="ListFrame.location.href='smsgroup_addresslist.asp?smsgroupid=<%=smsgroupid%>&smsgroupname=개인-친구';">
				...
				<tr onmouseover="this.style.background='#FFFCE7'" bgcolor="#FFFFFF" onmouseout="this.style.background='#FFFFFF'">
					...
					<td align="center">사용</td>
					<td align="center" width="60">
							<img src="/Images/Comm/IconHome.gif" title="목록보기" style="cursor:hand;" align="absmiddle" onclick="ListFrame.location.href='smsgroup_addresslist.asp?smsgroupid=<%=smsgroupid%>&smsgroupname=공통-긴급발령';">
							<img src="/Images/Btn/BtnIconModify.gif" title="그룹수정" style="cursor:hand;" align="absmiddle" onclick="javascript:fn_update('1111','UP');">
							<img src="/Images/Btn/BtnIconDel.gif" title="그룹삭제" style="cursor:hand;" align="absmiddle" onclick="javascript:fn_update('1111','DEL');">
					</td>
				</tr>
		...
		<td width="10"></td>
		<td width="590">
			<iframe src="smsgroup_addresslist.asp?smsgroupid=<%=idx%>&smsgroupname=<%=groupgb_NM%>-<%=groupname%>" name="ListFrame" width="100%" height="100%" frameborder=0 marginheight=0 marginwidth=0 scrolling="no"></iframe>
		</td>
...
%>
	...
	<form name="insFrm" method="post" action="smsgroup_move.asp">
...
<!--
	function fn_update(arg0, arg1){
		//sms그룹 수정
		ShowPOPLayer('smsgroup_detail.asp?idx='+arg0,'500','210');
	}
	function fn_del(arg0, arg1){
		//sms그룹 수정
		if (confirm("선택한 그룹을 삭제하시겠습니까?"))
			ShowPOPLayer('smsgroup_detail.asp?idx='+arg0+'&JOBGB=D','500','210');
	}
	function fn_insert(){
		//sms그룹 수정
